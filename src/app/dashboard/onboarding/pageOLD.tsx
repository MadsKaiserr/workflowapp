...
export default function Onboarding() {
    ...
    const [questions, setQuestions] = useState([
        ...
        },
        {
            ...
            sections: [
                ...
                },
                {
                    type: "input",
                    navn: "oneliner",
                    overskrift: "Skriv én linje, der fremhæver dig",
                    antalTegn: 0,
                    maxTegn: 60,
                    secondaryoverskrift: "",
                    text: "",
                    placeholder: "Skriv en rubrik, der beskriver dig"
                },
                {
                    type: "textarea",
                    navn: "personligbeskrivelse",
                    overskrift: "Fortæl os om dig selv",
                    antalTegn: 0,
                    maxTegn: 255,
                    text: "",
                    placeholder: "Skriv en beskrivelse om dig selv"
                },
                {
                    type: "dropdown",
                    navn: "by",
        ...
        {
            id: 5,
            navn: "Tid til at udfylde din profil",
            beskrivelse: "Tilpas din profil, som potentielle kunder ser",
            knapText: "Næste",
            skipable: true,
            quote: {
                quotation: "Workflow er en nordisk platform, som forbinder og matcher virksomheder med freelancere.",
            ...
            sections: [
                ...
                },
                {
                    ...
                    maxTegn: 1024,
                    text: "",
                    placeholder: "Skriv en detaljeret beskrivelse over din ydelser"
                }
            ]
        },
        {
            id: 6,
            navn: "Tilføj pakkeløsninger til din service",
            beskrivelse: 'Tilføj standardpakken som en pakkeløsning',
            knapText: "Færdig",
            skipable: true,
            quote: {
                quotation: "Workflow er en nordisk platform, som forbinder og matcher virksomheder med freelancere.",
        ...
        if (!svar.chosen) {
            if (sectionType != "selection") {
                question.sections[0].svar.forEach((allSvar: any, allSvarIndex: number) => {
                    ...
                    document.getElementById("q" + question.id + "-" + "s" + sectionId + "-" + "a" + allSvarIndex)?.classList.remove("opretkonto__element__active")
                })
                svar.chosen = true
                document.getElementById("q" + question.id + "-" + "s" + sectionId + "-" + "a" + svarId)?.classList.add("opretkonto__element__active");
            } else {
                let chosencount = 0
                ...
                if (chosencount < 5) {
                    svar.chosen = true
                    document.getElementById("q" + question.id + "-" + "s" + sectionId + "-" + "a" + svarId)?.classList.add("welcome__selection__tag__element__active");
                }
            }
        } else if (svar.chosen && sectionType == "selection") {
            svar.chosen = false
            if (sectionType == "selection") {
                document.getElementById("q" + question.id + "-" + "s" + sectionId + "-" + "a" + svarId)?.classList.remove("welcome__selection__tag__element__active");
            } else {
                document.getElementById("q" + question.id + "-" + "s" + sectionId + "-" + "a" + svarId)?.classList.remove("opretkonto__element__active");
            }
        }
    }
            ...
            <div className="welcome__content">
                {questions.map((question) => {
                    return (<div key={"q" + question.id} id={"q" + question.id} className="question__container">
                        <h1 className="question__h1">{question.navn}{user && (question.id == 0 || question.id == 2) && ", " + user.displayName}</h1>
                        {question.id === 1 ? <p className="question__p">Andre bruger i gennemsnit <span className="question__p__bold">kun 7 minutter</span> på at oprette en profil</p> : <p className="question__p">{question.beskrivelse}</p>}
                        {question.id >= 1 && <div className="welcome__status__container welcome__small__section">
                            <div className={question.id >= 1 ? "welcome__status__element welcome__status__element__active" : "welcome__status__element"}></div>
                            <div className={question.id >= 2 ? "welcome__status__element welcome__status__element__active" : "welcome__status__element"}></div>
                            <div className={question.id >= 3 ? "welcome__status__element welcome__status__element__active" : "welcome__status__element"}></div>
                            <div className={question.id >= 4 ? "welcome__status__element welcome__status__element__active" : "welcome__status__element"}></div>
                            <div className={question.id >= 5 ? "welcome__status__element welcome__status__element__active" : "welcome__status__element"}></div>
                            <div className={question.id >= 6 ? "welcome__status__element welcome__status__element__active" : "welcome__status__element"}></div>
                        </div>}
                        <ul className="welcome__form__container">
                            {question.sections?.map((section, sectionIndex) => {
                                if (section.type == "card-column") {
                                    return (<ul key={question.id + "-" + section.type + "-" + sectionIndex} className="opretkonto__container welcome__ned__container">
                                        {Array.isArray(section.svar) && section.svar.map((answer, answerIndex) => {
                                            return (<li key={"a" + answerIndex} id={"q" + question.id + "-" + "s" + sectionIndex + "-" + "a" + answerIndex} className="opretkonto__element" onClick={() => chooseCard(section.type, question.id, sectionIndex, answerIndex)}>
                                    ...
                                    return (<div className="welcome__selection__container" key={question.id + "-" + section.type + "-" + sectionIndex}>
                                        ...
                                        <ul className="welcome__selection__tag__container">
                                            {section.svar.map((tag, tagIndex) => {
                                                return <div key={question.id + "-" + sectionIndex + "-tag" + tagIndex} id={"q" + question.id + "-" + "s" + sectionIndex + "-" + "a" + tagIndex} className="welcome__selection__tag__element" onClick={() => chooseCard(section.type, question.id, sectionIndex, tagIndex)}>{tag.overskrift}</div>
                                            })}
                                        </ul>
                                    </div>)
                                } else if (section.type == "dropdown") {
                                        ...
                                        <div className="welcome__dropdown__element__container">
                                            <div className="welcome__dropdown__element__dropdown__container">
                                                <input className="welcome__dropdown__element__dropdown__input" onFocus={() => {
                                                    document.getElementById("wrapper" + question.id + "dropdown-" + sectionIndex)?.classList.add("welcome__dropdown__element__dropdown__wrapper__active")
                                                }} autoComplete={"off"} id={question.id + "dropdown-" + sectionIndex} placeholder={"Vælg " + section.navn} onChange={(e) => {
                                                    const searchedDupli = []
                                                    
                                                    questions[question.id].sections[sectionIndex].list.forEach((listItem) => {
                                                    ...
                                                    const questionsDupli = [...questions]
                                                    questionsDupli[question.id].sections[sectionIndex].searchedList = searchedDupli
                                                    setQuestions(questionsDupli)
                                                }} />
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
                                                    <path fillRule="evenodd" d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708"/>
                                                </svg>
                                                <div className="welcome__dropdown__element__dropdown__wrapper" id={"wrapper" + question.id + "dropdown-" + sectionIndex}>
                                                    {section.searchedList.map((item) => {
                                                        return (<button className="welcome__dropdown__element__dropdown__wrapper__element" key={section.navn + item.navn} onClick={() => {
                                                            const questionsDupli = [...questions]
                                                            questionsDupli[question.id].sections[sectionIndex].selected = item.navn

                                                            if (section.navn == "hovedkategori") {
                                                                setCurrentBranche(getKategorier().findIndex((itemFind) => itemFind.navn == item.navn))
                                                            }

                                                            document.getElementById(question.id + "dropdown-" + sectionIndex).value = item.navn
                                                            document.getElementById("wrapper" + question.id + "dropdown-" + sectionIndex)?.classList.remove("welcome__dropdown__element__dropdown__wrapper__active")
                                                            setQuestions(questionsDupli)
                                                        }}>
                                    ...
                                    return (<div className="welcome__selection__container" key={question.id + "-" + section.type + "-" + sectionIndex}>
                                        ...
                                        <div className="welcome__dropdown__element__container">
                                            <div className="welcome__dropdown__element__dropdown__container">
                                                <input className="welcome__dropdown__element__dropdown__input" autoComplete={"off"} id={question.id + "dropdown-" + sectionIndex} placeholder={section.placeholder} onChange={(e) => {
                                                    const questionsDupli = [...questions]
                                                    questionsDupli[question.id].sections[sectionIndex].text = e.target.value
                                                    setQuestions(questionsDupli)
                                                }} />
                                            </div>
                                        </div>
                                    ...
                                    return (<div className="welcome__selection__container" key={question.id + "-" + section.type + "-" + sectionIndex}>
                                        <p className="logind__form__element__heading">{section.overskrift}<span className={section.antalTegn == section.maxTegn ? "logind__form__element__heading__error" : ""}>{"(" + section.antalTegn + "/" + section.maxTegn + " tegn)"}</span></p>
                                        <div className="welcome__dropdown__element__container">
                                            <textarea className="welcome__dropdown__element__textarea" maxLength={section.maxTegn} autoComplete={"off"} id={question.id + "dropdown-" + sectionIndex} placeholder={section.placeholder} onChange={(e) => {
                                                const questionsDupli = [...questions]
                                                questionsDupli[question.id].sections[sectionIndex].text = e.target.value
                                                questionsDupli[question.id].sections[sectionIndex].antalTegn = e.target.value.length
                                                setQuestions(questionsDupli)
                                            }} />
                                        </div>
                                    </div>)
                                } else if (section.type == "price") {
                                    return (<ul key={question.id + "-" + section.type + "-" + sectionIndex} className="opretkonto__priser__container">
                                            ...
                                            return (<li key={pakke.navn + "-" + pakkeIndex} className="opretkonto__priser__element">
                                                ...
                                                <div className="opretkonto__priser__element__price">
                                                    ...
                                                    <input type="number" className="opretkonto__priser__element__price__input" placeholder={"00,00"} onChange={(e) => {
                                                            const questionsDupli = [...questions]
                                                            questionsDupli[question.id].sections[sectionIndex].pakker[pakkeIndex].price = parseFloat(e.target.value)
                                                            setQuestions(questionsDupli)
                                                        }} />
                                                </div>
                                                <div className="welcome__selection__container">
                                                    ...
                                                    <div className="welcome__dropdown__element__container">
                                                        <textarea className="welcome__dropdown__element__textarea" maxLength={pakke.maxTegn} autoComplete={"off"} id={question.id + "dropdown-" + sectionIndex} placeholder={"Skriv en beskrivelse til pakken..."} onChange={(e) => {
                                                            const questionsDupli = [...questions]
                                                            questionsDupli[question.id].sections[sectionIndex].pakker[pakkeIndex].beskrivelse = e.target.value
                                                            questionsDupli[question.id].sections[sectionIndex].pakker[pakkeIndex].antalTegn = e.target.value.length
                                                            setQuestions(questionsDupli)
                                                        }} />
                                                    </div>
                                                </div>
